...
#define EPSILON 1e-8
#define CLIP_GRAD 10.0 // Gradient clipping limit
...
enum lossfunc { MSE, BCE, HUBER}; // Enum for loss functions
...
class Neuron {
    private:
        ...
        double output, target, bias, 
            error, error_Lw, error_Lb, prediction, 
            gradient_bias, 
            learning_rate, decay_rate; 

        //batch parameters and normalization
        bool use_batch_norm; // Flag for batch normalization
        double running_mean, running_var; // Batch normalization parameters
        double bn_gamma, bn_beta; // Trainable parameters for batch normalization
        ...
        void loss_derivative() {
            switch (lossFunc) {
                case MSE:
                    error = target - prediction; // Mean squared error
                    break;
                case BCE:
                    //error = (target - prediction) / (prediction * (1 - prediction)); //Binary cross entropy
                    //error = (prediction - target)/(prediction * (prediction - 1)); //Binary cross entropy
                    error = -(target / prediction) + ((1 - target) / (1 - prediction)); //Binary cross entropy
                    break;
                case HUBER:
                    double delta = 0.05; // Huber loss delta
                    if (abs(target - prediction) <= delta) {
                        error =  target - prediction;  // MSE region
                        break;
                    } else {
                        error = delta * (target - prediction > 0 ? 1 : -1);  // MAE region
                        break;
                    }
            }
        }
    ...
    public:
        ...
        Neuron(const vector<double>& inputs, double targets, 
        ...
        void normalization(){
            ...
            if(actFunc == RELU || actFunc == LEAKY_RELU || actFunc == ELU || actFunc == SIGMOID){
                for (size_t i = 0; i < input.size(); i++){
            ...
            else if (actFunc == RELU || actFunc == LEAKY_RELU || actFunc == ELU || actFunc == TANH){
                double mean = accumulate(input.begin(), input.end(), 0.0) / input.size();
                for (size_t i = 0; i < input.size(); i++){
                    input[i] = (input[i] - mean)/(max - min);
                }
                target = (target - mean)/(max - min);
            }
        }
        ...
        void feedforward() {
            ...
            this -> prediction = activation_value(output);
            loss_derivative(); //Calculate the error
        }
        ...
        void backward() {
            ...
            //this -> beta = exp(-0.01 * timestep);

            #pragma omp parallel for 
            //Updating the weights
            for (size_t i = 0; i < input.size(); ++i) {

                gradient_weight[i] = (-(error + error_Lw) * activation_derivative(output) * input[i]);
        ...
        void set_use_batch_norm(bool use) {this -> use_batch_norm = use;}  //Setting the batch normalization flag
};
